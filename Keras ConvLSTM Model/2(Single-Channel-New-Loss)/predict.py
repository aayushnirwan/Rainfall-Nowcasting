...
model = load_model("loss.0.6056-004.h5", custom_objects={'focal_tversky': new_loss.focal_tversky})
...
#     pred = np.squeeze(pred,axis=-1)
    ...
    # x = np.squeeze(x,axis=-1)
    # print (np.unique(x,return_counts=True))
    # cv2.imwrite("results/{} (10).jpeg".format(i),x)
    #
    # for j in range(10):
    #     y = val_input[i][j]
    ...
    # x=np.where(np.argmax(val_input[i][9],axis=2)==0,255,0)
    # cv2.imwrite("new/{}.jpeg".format(i),x)
