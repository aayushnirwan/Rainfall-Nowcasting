...
# train_input = np.load('train_sequences_400_all1.npy')
# train_label = np.load('train_labels_400_all1.npy')
# val_input = np.load('val_sequences_400_all1.npy')
# val_label = np.load('val_labels_400_all1.npy')
train_input = np.load('ti.npy')
train_label = np.load('tl.npy')
val_input = np.load('vi.npy')
val_label = np.load('vl.npy')

train_input = np.where(train_input>0,1,0)
train_label = np.where(train_label>0,1,0)
val_input = np.where(val_input>0,1,0)
val_label = np.where(val_label>0,1,0)
...
# model.fit(x=train_input[0:1,:,:,:,:], y=train_label[0:1,:,:,:], batch_size=1, epochs=300, verbose=1, validation_data=(val_input[0:1,:,:,:,:], val_label[0:1,:,:,:]), shuffle=True)
